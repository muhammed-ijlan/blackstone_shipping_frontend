...
const SupportBannerContent = ({data}:{data:SupportPageData}) => {
  return (
    <Stack direction={{xs:"column-reverse",md:"row"}} gap={3} sx={{alignItems:"center"}}>
        <Stack width={"100%"}>
            <Typography variant='h1' sx={{textTransform:"capitalize !important"}} color="rgba(45, 55, 72, 1)">{data.page.supportPageContentSection.mainTitle.split("<br/>")[0]}</Typography>
            <Typography variant='h1' sx={{textTransform:"capitalize !important"}} color="rgba(45, 55, 72, 1)">{data.page.supportPageContentSection.mainTitle.split("<br/>")[1]}</Typography>
